...
function CardAdminDaerah({ handleCheckboxChange , searchTerm =''}) {
    ...
    const handleFoto = async () => {
        try {
            ...
            const response = await axios.post(
                `${BASE_URL}/api/adminupfoto/${selectedItemId}`,
                formData,
                {
                    headers: {
    ...
    const selectedItem = barangData.find((item) => item.id_barang === selectedItemId);
         ...
         {Array.isArray(filteredData) && filteredData.length > 0 ? (
          <div style={{ width: '20rem' }}>
              {filteredData.map((data, index) => (
                  <Card key={index} style={{ marginBottom: '20px' }} onClick={() => handleShowModal(data.id_barang)}>
